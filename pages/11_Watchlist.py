...
st.caption(
    ...
    "Watchlist is synced to your account and accessible across all your devices."
)
...
with ctrl_right:
    st.markdown("")
    st.markdown("")
    refresh_btn = st.button("🔄 Refresh scores", width='stretch',
...
for idx, ticker in enumerate(all_tickers_for_manage):
    ...
    with remove_cols[idx % len(remove_cols)]:
        ...
        if st.button("✕", key=f"rm_{ticker}", width='stretch',
...
_weights_ready = abs(_saved_total - 100.0) < 0.5 and len(_saved_weights) > 0
...
# ── Render rebalancing plan if it exists ──────────────────────────────────────
